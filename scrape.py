...
def init_folders():
	...
	if not os.path.exists("Sites/" + firma):
		os.makedirs("Sites/" + firma)

###############
### SCRAPER ###
###############

def get_documents (s = suchbegriff, b = bereich, l = language, sd = start_day, sm = start_month, sy = start_year, ed = end_day, em = end_month, ey = end_year):	
	...
	while page_next is not None:
		...
		doc_links = soup.find_all('a', attrs={'title': 'Stimmrechtsmitteilungen'})
...
def parse_document(doc, doc_name):
	...
	if bool(re.search("Inhalt der[\n \s]*Stimmrechtsmitteilung:?[\n \s]*" + suchbegriff, tableText, re.I)): return "selber_emittent"
	...
	for f in os.listdir("Sites/"):
		if f[:2] == 'id':
			...
			if result == False:
				...
				os.rename("Sites/" + f, "Sites/" + firma + "/" + f)
			# zweiter Durchlauf
